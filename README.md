# Concierge MCP Server + Access OAuth
...
- Exposes a `read_webpage_as_markdown` tool backed by Cloudflare Browser Run
...
The Worker uses a Browser Run binding named `BROWSER`; no separate Browser Run API
token is required for the tool.
...
Once the Tools show up in the interface, you can ask Claude to use them. For example: "Use `read_webpage_as_markdown` to read `https://example.com`." Claude should invoke the tool and show the Markdown generated by the MCP server.
